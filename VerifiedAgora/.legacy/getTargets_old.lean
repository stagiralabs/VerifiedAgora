...
/-
TODO:
If target = none, don't build descriptors for both submission and target
just build submission and run against itself and exit early (compare_self flag)

Don't get tagged with new env'' if we don't have to. ideally only do one import
don't cycle through all constants, rather, just do the ones in the module.
