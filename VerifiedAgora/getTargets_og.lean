    ...
    for (n, ci) in scanCandidates do
      if let some rng := humanDeclMap.get? n then
        ret := ret.push ({
          ...
          modified? := false, -- we fill this in later after comparing to target
      ...
      for target in (targetDescriptor?.getD default).decls do
        if (humanDeclMap.get? target.ci.name).isSome then
          if let some ci'_info := ciMap.get? target.ci.name then
            let ci' := .fromConstantInfo mod ci'_info --ci' is from submission, target.ci is from target
...
unsafe def getTargets' (timingsRef : IO.Ref TimingState)
  ...
  if target_module == submission_module then
    ...
    let (targetDescriptor, _) ← (match extractedTargetDescriptor? with
      | some desc => pure (desc, #[])
      | none => do
    ...
    let (submittedDeclDescriptors, submissionFailures) ← getDescriptorForModule timingsRef submission_module targetDescriptor
    let submittedDescriptor : FileDescriptor := {
      decls := submittedDeclDescriptors,
      path := submissionSourceFile,
      moduleName := submission_module,
      contents := submission_data.1
    }

    if automerge then do
      -- for each decl in targetDescriptor, find the matching decl in submittedDescriptor (if any). If it is resolved, and that decl isn't in the failures, then merge it.
      let mut contents := targetDescriptor.contents
      let mut decls := #[]
      for decl in targetDescriptor.decls do
        let matchingSubmittedDecl? := submittedDescriptor.decls.find? (fun d => d.ci.name == decl.ci.name)
        if let some matchingSubmittedDecl := matchingSubmittedDecl? then
          let failure? := submissionFailures.find? (fun f => f.declName == decl.ci.name.toString)
          if matchingSubmittedDecl.resolved? && failure?.isNone then
            decls := decls.push matchingSubmittedDecl
            -- we update contents by replacing the old decl contents with the new one. This is a bit hacky but it allows us to preserve comments and formatting in the unchanged parts of the file, which is nice.
            contents := contents.replace decl.contents matchingSubmittedDecl.contents
    ...
    if submissionFailures.size > 0 then
      return (targetDescriptor, submissionFailures)
    else
      return (submittedDescriptor, submissionFailures)
...
unsafe def getTargetsCLI (args : Cli.Parsed) : IO UInt32 := do
  ...
  let automerge ← parseBoolFlag "automerge" (args.flag! "automerge" |>.as! String)
  ...
  try
    ...
    let (descriptor, failures) ← getTargets' timingsRef submissionContent targetContent? useCache automerge
...
"Get targets from a string."
  ...
  FLAGS:
    ...
    automerge : String; "If true, then we attempt to merge resolved, correct submission decls into the target file even when the full submission is not entirely correct. Default: true."
    save : String; "If provided, save the file descriptor to this file as json to specified path. Default is no save."

  EXTENSIONS:
    defaultValues! #[("target", ""), ("save", ""), ("use_cache", "true"), ("automerge", "true")]
]
